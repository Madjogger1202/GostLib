...
- **Экспорт в проект KiCad** — правым кликом: символ, посадка и 3D-модель
  дописываются в библиотеки проекта и подключаются к нему. Символы, нарисованные
  руками, остаются на месте, повторный экспорт не плодит дублей.
- **Цветная 3D-модель из EasyEDA** — берётся родной STEP производителя, а не
  перегнанная сетка; ставится на место по сетке той же модели.
- **Правила выводов для микроконтроллеров из коробки** — около сорока групп:
  порты A…K, GPIO, USB, Ethernet, SPI, I2C, UART, SD, QSPI, I2S, DDR, питание,
  тактирование, отладка. На живом каталоге под них попадает 99 % выводов против
  трети раньше. Порты сами делятся между сторонами, земля внизу слева.
  «Проверить на этом компоненте» показывает раскладку до применения.
- **«Сохранить 3D-модель…»** — правым кликом.
- **Сдвиг и поворот 3D — полями**, а не десятками нажатий на «+0,1».
- **Панель разгружена**: «импорт в проект», запасные пути и служебные действия
  переехали в настройки; кнопка «Собрать задание» по умолчанию спрятана (`F9`
  работает).

## Что было в 1.5

- **Родное УГО из EasyEDA/LCSC** — обозначение из источника сохраняется и
  выбирается так же, как при импорте из KiCad: двунаправленный супрессор
  приезжает супрессором, а не обычным диодом.
- **Посадочные места из чужих `.PcbLib` читаются целиком** — площадки,
  шелкография, дуги, заливки. Видно в превью, в габаритах и под 3D-моделью.
- **Просмотр 3D больше не рассыпается** на сложных корпусах: разъём на сорок
  контактов показывается целиком, а не кашей из сваренных вместе контактов.
- **Архив производителя не пропадает**, если библиотека Altium в нём не
  прочиталась: компонент берётся из файлов KiCad в том же архиве.
- **Угол овального отверстия** считается от площадки, а не от платы — пазы
  крепления USB-разъёмов из KiCad перестали ложиться поперёк.
- **Версия проставляется в свойствах `exe`** — видно, что за сборка, не
  запуская её.
...
## Обновление и где лежат данные

**Обновляться безопасно: библиотеку это не бьёт.** Программа и данные лежат
раздельно. Обновляется только `GostLib.exe`, а каталог компонентов, настройки,
3D-модели и резервные копии живут в рабочей папке и не переписываются — и не
удаляются вместе с программой. Установщик ставится поверх (Windows видит в нём
обновление), а при удалении отдельно спрашивает, трогать ли рабочую папку.

Рабочая папка по умолчанию — **`%LOCALAPPDATA%\GostLib`**, то есть
`C:\Users\<имя>\AppData\Local\GostLib`:

| Путь | Что там |
|---|---|
| `catalog.sqlite` | каталог компонентов — главное, что стоит беречь |
| `settings.json` | настройки: стиль ЕСКД, шрифт, пути |
| `library\` | собранные `.SchLib` / `.PcbLib` (если не задана своя папка) |
| `models\` | 3D-модели |
| `backups\` | резервные копии, «Настройки → Сервис → Резервная копия…» |
| `cache\`, `jobs\` | скачанное из EasyEDA и задания для Altium — удаляются без потерь |

Папку можно унести на другой диск или в облачную папку отдела — переменной
окружения `GOSTLIB_HOME`:

```bat
setx GOSTLIB_HOME D:\GostLibData
```

Какая папка используется сейчас, программа пишет первой строкой в журнал при
запуске. Подробности — [docs/УСТАНОВКА.md](docs/УСТАНОВКА.md).
