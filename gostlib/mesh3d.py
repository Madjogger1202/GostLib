...
# Потолок подробности для просмотра.
#
# Здесь стояло 5000. Для QFN этого хватает, а разъём на 40 контактов
# (FH12-40S, шаг 0,5 мм) при таком бюджете рассыпается: клеточное
# упрощение сваривает соседние контакты между собой -- ячейка выходит
# крупнее шага, и вместо гребёнки контактов получается каша, сквозь
# которую видно изнанку корпуса. Ровно это и выглядит как «какие-то
# поверхности, не целостная модель».
#
# Сетку такого размера окно тянет: при вращении рисуется только часть
# граней (view3d.DRAG_FACES), а в покое -- все.
MAX_TRIS = 30000

# Во сколько раз огрублять допуск на каждом заходе и сколько заходов
# делать. Само число треугольников с ростом допуска выходит на полку
# (у сложной детали она около 28 тысяч): гнать дальше бессмысленно,
# поэтому останавливаемся, когда очередной заход почти ничего не убавил.
TOL_GROWTH = 4.0
TOL_TRIES = 4
TOL_STALL = 0.15
...
def load(step_path: str, cache_dir: str = "", tol: float = 0.05
    ...
    # остаётся целой, треугольников становится меньше. Огрубление бьёт
    # только по скруглениям, а плоские грани остаются как есть -- поэтому
    # оно НАМНОГО безопаснее клеточного упрощения, и клеточное остаётся
    # крайним средством.
    parts, scale = [], 1.0
    cur = tol
    total = 0
    for attempt in range(TOL_TRIES):
        try:
        ...
        was, total = total, sum(len(f) for _v, f, _c in parts)
        if total <= MAX_TRIS or ext not in (".step", ".stp"):
            break
        # полка: дальше грубить нечего, останется клеточное упрощение
        if attempt and total > was * (1.0 - TOL_STALL):
            break
        cur *= TOL_GROWTH
